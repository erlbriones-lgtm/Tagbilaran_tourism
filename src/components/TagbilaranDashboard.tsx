              ...
              {tagbilaranBarangays.map((brgy, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    className="cursor-pointer group relative overflow-hidden bg-gradient-to-br from-[#2ee59d] via-[#38B000] to-[#006400] border border-[#32e875]/30 hover:border-[#32e875] rounded-3xl p-6 sm:p-8 flex flex-col items-center justify-between min-h-[380px] sm:min-h-[420px] transition-all duration-300 shadow-xl hover:shadow-[#32e875]/25 hover:-translate-y-1.5"
                    id={`barangay-box-${brgy.name.replace(/\s+/g, "-")}`}
                  >
                    {/* Top Decorative Forest Green / Lime Accent Bar */}
                    <div className="absolute top-0 inset-x-0 h-1.5 bg-gradient-to-r from-emerald-500/10 via-[#32e875]/40 to-emerald-500/10 group-hover:via-[#32e875] transition-colors duration-300" />

                    {/* Logo Container (perfectly centered, fixed height for seamless vertical positioning) */}
                    <div className="flex-1 flex items-center justify-center w-full min-h-[240px] sm:min-h-[265px] py-4">
                      <img
                        src={logoImg}
                        alt={`${brgy.name} Logo`}
                        className={`${getGridLogoSize(brgy.name)} object-contain transition-all duration-500 select-none`}
                        referrerPolicy="no-referrer"
                      />
                    </div>

                    {/* Subtle Horizontal Sepatator */}
                    <div className="w-full h-[1px] bg-white/20 my-4 group-hover:bg-[#32e875]/30 transition-colors duration-300" />

                    {/* Lettering / Name Container */}
                    <div className="w-full flex items-center justify-center min-h-[50px] sm:min-h-[60px]">
                      {letteringImg ? (
                        <img
                          src={letteringImg}
                          alt={`${brgy.name} Lettering`}
                          className={`${getGridLetteringSize(brgy.name)} w-auto max-w-[85%] object-contain transition-all duration-500 select-none`}
                          style={{
                        ...
                        <h2 className="font-sans text-xl sm:text-2xl font-black text-white tracking-tight uppercase leading-none transition-all duration-500 group-hover:scale-105 group-hover:text-[#FFD54F]">
                  ...
                  {[...tagbilaranBarangays, ...tagbilaranBarangays, ...tagbilaranBarangays].map((otherBarangay, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isActive && (
                          <div className="absolute top-4 right-4 flex items-center gap-1 bg-[#FFD54F] text-black text-[9px] font-sans tracking-widest px-2.5 py-1 rounded-md font-black uppercase shadow-lg z-20 animate-fade-in">
